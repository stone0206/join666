...
<body>
	...
	<nav class="navbar navbar-light bg-light">
		<div class="container-fluid">
						...
						<p class="dropdown-item">
							...
							<c:forEach var="typescontent" items="${types }">
								<button id="search-btn">
									<a href="/searchByTypeId?id=${typescontent.id
												}">${typescontent.type
													}</a>
								</button>
							</c:forEach>
					...
					<li>
						...
						<form method="post" action="/searchByHLPrice">
							<p class="dropdown-item" style="margin-right: 10px;">
								價格區間： <input type="text" style="width: 60px;" placeholder="最低價"
									name="lowPrice"> 一 <input type="text"
									style="width: 60px; margin-right: 10px;" placeholder="最高價"
									name="highPrice"> <input type="submit" value="送出">
							</p>
						</form>
					</li>
				</ul>
			...
			<!-- 搜尋表單 -->
			<form class="d-flex" id="search-form" action="/searchByKey"
				method="post">
				<!-- 搜尋框 -->
			...
			<div class="dropdown">
				<a class="btn btn-secondary dropdown-toggle" role="button"
					id="dropdownMenuLink" data-bs-toggle="dropdown"
					aria-expanded="false" style="width: 160px; border-radius: 38px;">
					排序方式 </a>
				<!-- 排序方式 -->
				<ul class="dropdown-menu" aria-labelledby="dropdownMenuLink">
	...
	<!-- 查看詳情 -->
	<c:forEach var="p" items="${products}">
		<div class="modal fade" id="staticBackdropbydetail${p.id }"
	...
	<button type="button" data-bs-toggle="modal"
		data-bs-target="#staticBackdrop" id="shopping-cart">
		<img src="${contextRoot }/assets/shopping-car-icon.png"
			style="width: 60px" onclick="openShoppingCart()" )>
	</button>
		...
		<div class="modal-dialog modal-dialog-scrollable modal-lg">
			<div class="modal-content">
				...
				<div class="modal-footer">
					<!-- 取消結帳繼續選購 -->
					<button type="button" class="btn btn-secondary"
						data-bs-dismiss="modal">繼續選購</button>
					<!-- 購物車訂單確認結帳 -->
	...
	<script>
					...
					function openShoppingCart() {
						fetch('${contextRoot}/openShoppingCart').then(res => res.json()).then(json => {
							let shoppingCartData = '';
								$.each(json, function (index, value) {
									shoppingCartData += '<tr><td><div id="imgbox"><img src='
									shoppingCartData += value.product.img + '" alt=""></div></td>'
									shoppingCartData += '<td>' + value.product.name + '</td>'
									...
									shoppingCartData += '<td>' + '<a href="#">刪除</a>' + '</td>'
									shoppingCartData += '</tr>'
								})
